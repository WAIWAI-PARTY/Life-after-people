...
export(Array) var damage = [0,0]
export(Array) var bullet_speed = [0,0]
export(Array) var bullet_health = [0,0]
...
onready var cam_shake = get_node("Camera2D/shake")
...
func _process(_delta):
	if Input.is_action_pressed("shoot") and can_fire:
		if !cam_shake.is_shaking:
			cam.offset = lerp(cam.offset, (Vector2.RIGHT*10).rotated(rotation), 0.5)
			shaketimer.start()
		if shootCount >= magazineVol:
			checkReload()
		else:
			...
			bullet_instance.damage = damage[current_bullet]
			bullet_instance.speed = bullet_speed[current_bullet]
			bullet_instance.health = bullet_health[current_bullet]
